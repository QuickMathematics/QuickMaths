...
const AUTHORED_MATH_SCENARIO_COUNTS = Object.freeze({
  MATH_ARITH_006: 20, MATH_ARITH_007: 20, MATH_ARITH_008: 20, MATH_ARITH_009: 20, MATH_ARITH_010: 20, MATH_ARITH_011: 20, MATH_ARITH_012: 20, MATH_ARITH_013: 20, MATH_ARITH_014: 20, MATH_ALG_009: 20, MATH_GEOM_005: 20, MATH_GEOM_006: 20, MATH_GEOM_007: 20, MATH_GEOM_008: 20, MATH_GEOM_009: 20, MATH_GEOM_010: 20, MATH_GEOM_011: 20, MATH_GEOM_012: 20, MATH_ALG_010: 20, MATH_RAD_002: 20,
  MATH_LOG_001: 12, MATH_LOG_002: 13, MATH_LOG_003: 13,
  MATH_TRIG_001: 12, MATH_TRIG_002: 12, MATH_TRIG_003: 13,
  MATH_TRIG_004: 13, MATH_TRIG_005: 13, MATH_TRIG_006: 16,
  ...
  MATH_ARITH_004: 22, MATH_ARITH_005: 25, MATH_GEOM_004: 21,
  ...
  MATH_FUNC_001: 10, MATH_FUNC_002: 20, MATH_FUNC_003: 20, MATH_FUNC_004: 20, MATH_FUNC_005: 20,
  MATH_FUNC_006: 20, MATH_FUNC_007: 20, MATH_CALC_001: 20, MATH_CALC_002: 20, MATH_EXP_001: 10, MATH_RAD_001: 10,
...
test("ships the complete native Mathematics curriculum and opens at the profile picker", () => {
  ...
  assert.equal(curriculum.skills.length, 116);
  assert.equal(curriculum.skills.reduce((count, skill) => count + skill.question_count, 0), 1761);
  assert.ok(curriculum.skills.reduce((count, skill) => count + skill.problems.length, 0) > 1761);
  assert.equal(curriculum.skills.filter((skill) => skill.subjectId === "SUBJECT_GEOGRAPHY").length, 0);
  assert.equal(curriculum.skills.filter((skill) => skill.id.startsWith("MATH_GEOM_")).length, 12);
  assert.deepEqual(state.subjects.map((subject) => [subject.id, subject.skillIds.length]), [["SUBJECT_MATH", 116]]);
...
test("Mathematics catalogs retain ordinary and added formal scenarios independently of assessment length", () => {
  assert.equal(Object.values(AUTHORED_MATH_SCENARIO_COUNTS).reduce((total, count) => total + count, 0), 1723);
  const assessmentLengths = { MATH_FUNC_004: 21, MATH_FUNC_007: 21, MATH_CALC_001: 21, MATH_CALC_002: 21,
  MATH_CALC_003: 21,
  MATH_CALC_004: 21,
  MATH_CALC_005: 21,
  MATH_CALC_006: 21,
};
  for (const [skillId, ordinaryScenarioCount] of Object.entries(AUTHORED_MATH_SCENARIO_COUNTS)) {
    const skill = curriculum.skills.find((candidate) => candidate.id === skillId);
    assert.ok(skill, `${skillId} must ship`);
    assert.equal(skill.question_count, assessmentLengths[skillId] ?? ordinaryScenarioCount, `${skillId} assessment length`);
    const ordinary = skill.native_templates.filter((template) => !template.proof_spec);
    assert.equal(ordinary.length, ordinaryScenarioCount, `${skillId} ordinary runtime template coverage`);
    ...
    assert.ok(skill.problems.length >= skill.question_count, `${skillId} needs a complete question bank`);
  }
});
...
test("Depot Geography is substantial, installable, and bridges through native Mathematics", () => {
  ...
  assert.equal(installed.totalSkillCount, curriculum.skills.length + 15);
...
test("educator curricula isolate packs, export canonical plans, and keep practice repeatable", () => {
  ...
  assert.equal(store.snapshot().curriculum.allSkills.length, curriculum.skills.length + 15);
  store.setCurriculumPackEnabled("PACK_GEOGRAPHY", false);
  assert.equal(store.snapshot().curriculum.allSkills.length, curriculum.skills.length);
  ...
  assert.equal(state.curriculum.allSkills.length, curriculum.skills.length);
...
test("saved five-question drafts expand in place without losing existing answers", () => {
  ...
  saved.version = 4;
  delete legacyDraft.snapshot_version;
  delete legacyDraft.snapshot_sha256;
  delete legacyDraft.assessment_variation;
...
test("a valid custom lesson set joins the real curriculum without replacing built-in skills", () => {
  ...
  assert.equal(store.snapshot().progressRows.length, curriculum.skills.length, "preview must not mutate state");

  const installed = store.importLessonPack(lessonSetExample);
  const state = store.snapshot();
  assert.equal(installed.totalSkillCount, curriculum.skills.length + 1);
  assert.equal(state.lessonPacks.length, 1);
  assert.equal(state.progressRows.length, curriculum.skills.length + 1);
...
test("native lesson improvements replace content reversibly without moving IDs or completed learner progress", () => {
  ...
  assert.equal(store.snapshot().curriculum.allSkills.length, curriculum.skills.length, "preview must not mutate the curriculum");
  ...
  assert.equal(installed.totalSkillCount, curriculum.skills.length);
  assert.equal(state.curriculum.allSkills.length, curriculum.skills.length);
...
test("native improvements enforce the built-in identity and round-trip through full backups", () => {
  ...
  assert.equal(restored.curriculum.allSkills.length, curriculum.skills.length);
...
test("subjects share one visible map, apply bridge locks, and theme the last opened lesson", () => {
  ...
  assert.equal(state.progressRows.length, curriculum.skills.length);
  assert.equal(state.allProgressRows.length, curriculum.skills.length + 1);
...
test("custom progress and content round-trip together through a full backup", () => {
  ...
  target.store.selectProfile(target.store.snapshot().profiles[0].id);
  const restored = target.store.snapshot();
  assert.equal(restored.progressRows.length, curriculum.skills.length + 1);
