...
test("all shipped lessons use the shared broad-branch standard", () => {
  ...
  assert.equal(new Set(entries.map(([field, skill]) => `${field}:${skill.id}`)).size, entries.length);
...
test("canonical nodes sit in non-overlapping field/branch bands and prerequisites stay left of dependents", () => {
  ...
  assert.equal(Object.keys(layout.positions).length, skills.length);
  assert.equal(layout.lanes.length, 3);
  assert.equal(layout.lanes.flatMap((lane) => lane.branches).length, 15);
  let end = 0;
