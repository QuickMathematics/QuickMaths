...
- Branch revision reads bypass HTTP caches. If an automatic history-only merge races before any choices appear, the app rebuilds the comparison from fresh copies, with at most two retries. If either remote revision or meaningful local content changes during a displayed review, refresh the comparison. Writes use GitHub's expected file SHA and never retry by overwriting an unseen revision. Failed writes leave local work intact.
