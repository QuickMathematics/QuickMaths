...
- Branch revision reads bypass HTTP caches. Saving compares the content behind the latest revisions with the copies used for review. Session clocks, audit entries, touch timestamps and unchanged-content checkpoint rewrites keep the chosen checkboxes valid; recent activity is combined automatically. Real edits, a different agent task base, or another device's resolved agent decision require a refreshed comparison. Writes use the latest checked file SHA. A write race gets at most two retries, each with fresh content checks; the app never forces an overwrite of unseen work. Failed writes leave local work intact.
