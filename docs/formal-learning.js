 ...
 */
import {
  changeFormalSubproof, appendFormalStep, assertFormalSessionForJob, beginFormalStepEdit, checkFormalProgress,
...
export function createFormalLearning({ read, write, archive, getArchive, options = {} }) {
  ...
  const identity = (item) => formalRequestHash({
    ...
    job: item.problem.formal_job, assessmentPolicy: item.problem.proof_spec.assessment_policy ?? {},
  ...
  async function run(questionId, action, input = {}) {
    ...
    if (!["start", "edit", "cancel", "append", "progress", "verify", "replay", "open_scope", "close_scope"].includes(action)) throw new Error("Unknown formal proof action.");
    if (["verify", "replay"].includes(action)) assertFormalMethod(item.problem.proof_spec.assessment_policy ?? {}, item.evidence?.request);
    ...
    try {
      ...
      if (action === "start") session = await startFormalProof(item.problem.formal_job, options);
        ...
        if (session.pendingEdit && !["append", "close_scope"].includes(action)) throw new Error("Save the edited step before checking the proof.");
        ...
        if (action === "open_scope" || action === "close_scope") session = await changeFormalSubproof(session, input, action === "close_scope", options);
      ...
      if (verification) assertFormalMethod(item.problem.proof_spec.assessment_policy ?? {}, session.request);
      ...
      return { ok: true, action, selected_scope: session.selectedScope ?? null, verification_status: session.verification?.status ?? session.progress?.status ?? "not_checked",
