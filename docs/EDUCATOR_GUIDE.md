...
## Teaching the derivative sequence

MATH_CALC_003 through MATH_CALC_006 add forty worked examples, sixteen applications, eighty ordinary scenarios and four formal exercises. Each 21-question assessment retains both its conceptual capstone and formal exercise. The capstone requires tutor review; that review cannot substitute for the formal question's fresh Lean verification. The derivative-definition task enforces its supported method policy. Rule-based later tasks establish their stated derivatives and do not claim to enforce a broader unauthored strategy.

Use the changing-area model, removable-hole comparison and nested square-root examples to discuss assumptions as well as computation. The sequence extends existing Calculus prerequisites without changing earlier locks. Lesson Studio preserves the figures, mathematical displays, original domain restrictions and the separate assessment requirements when creating native improvements.
