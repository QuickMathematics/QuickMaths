...
import { createLessonPublisher, PUBLISH_LICENSES, PUBLISH_TOKEN_URL } from "./lesson-publisher.js?v=20260906-media-v1";
...
export function createLessonPublisherDialog({ curriculum, getDraft, download, storage = globalThis.localStorage, documentImpl = globalThis.document, client = createLessonPublisher({ curriculum }) } = {}) {
  ...
  const selectPack = (value, files = new Map()) => {
    attachments = files;
  ...
  dialog.addEventListener("change", async (event) => {
    if (busy) return;
    if (event.target.matches("[data-publisher-folder]")) {
      if (event.target.files?.length) await run("Reading the lesson folder and its attachments…", async () => { const loaded = await readLessonFolder(event.target.files, { portable: false }); selectPack(loaded.pack, loaded.attachments); });
      return;
    }
    if (!event.target.matches("[data-publisher-file]")) return;
    const file = event.target.files?.[0];
    if (file) await run("Reading your lesson package…", async () => selectPack(await parseLessonManifest(await readFileTextLimited(file, 2_000_000, { label: "Lesson package" }), file.name)));
  });
  dialog.addEventListener("click", async (event) => {
    ...
    if (name === "review") return run("Validating the lesson and checking your public registry…", async () => {
      review = await client.prepare({ pack: { ...pack, version: version.trim() }, repo, license, attachments });
  ...
  return {
    ...
    clearDraft() { if (busy) return; pack = null; attachments = new Map(); review = null; result = null; status = null; render(); },
  };
}
