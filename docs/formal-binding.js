...
function canonicalJson(value) {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new Error("Formal binding cannot contain non-finite numbers.");
  ...
  throw new Error("Formal binding contains an unsupported value.");
}
...
export function formalProblemBinding(problem) {
  if (!problem?.proof_spec) throw new Error("Formal problem has no proof specification.");
  const payload = {
    skill_id: String(problem.skill_id ?? ""),
    template_id: String(problem.template_id ?? ""),
    seed: Number(problem.seed ?? 0),
    values: problem.values ?? {},
    prompt: String(problem.prompt ?? ""),
    proof_spec: problem.proof_spec,
  };
...
export function buildBoundFormalJob(problem, { maxSeconds = 60 } = {}) {
  ...
  return {
    ...
    rpc: {
      ...
      goal: String(spec.statement?.goal ?? ""),
...
export { canonicalJson as canonicalFormalJson };
