...
test("browser shell exposes Settings, Lesson Depot, map zoom, prompt copy, and persistent Agent Studio controls", () => {
  ...
  assert.match(js, /your original questions and working are preserved/);
...
test("educator WebMCP tools compose curricula and expose learner-visible supplemental guidance", async () => {
  ...
  assert.ok(store.snapshot().curriculum.allSkills.filter((skill) => skill.subjectId === "SUBJECT_MATH").length < curriculum.skills.length);
...
test("app, curriculum, and progress tools expose the full learner state", async () => {
  ...
  assert.equal(map.skills.length, curriculum.skills.length);
  assert.equal(summary.skills.length, curriculum.skills.length);
  assert.equal(summary.suggested_next.skill_id, "MATH_ARITH_006");
});
...
test("subject tools describe the combined curriculum and open the no-code creator", async () => {
  ...
  assert.deepEqual(nativeSubjects.subjects.map((subject) => [subject.subject_id, subject.skill_count]), [["SUBJECT_MATH", curriculum.skills.length]]);
  store.importLessonPack(geographyLessonSet);
  ...
  assert.deepEqual(installedSubjects.subjects.map((subject) => [subject.subject_id, subject.skill_count]), [
    ["SUBJECT_MATH", curriculum.skills.length], ["SUBJECT_GEOGRAPHY", 15],
  ]);
  ...
  assert.equal(combinedMap.skills.length, curriculum.skills.length + 15);
