...
const publicSource='module\npublic import Mathlib.Basic.Real.Basic\npublic theorem result : True := True.intro\n';
const curated={...matched,importPolicy:'production-module-public'};
assert.equal(sourceForExperiment(publicSource,curated).source,publicSource);
assert.deepEqual(sourceForExperiment(publicSource,curated).substitutions,[]);
assert.throws(()=>sourceForExperiment(publicSource,matched),/module mode/);
assert.throws(()=>sourceForExperiment(source,curated),/module mode/);
assert.throws(()=>sourceForExperiment('-- misleading comment\nmodule\n'+source,curated),/module mode/);
